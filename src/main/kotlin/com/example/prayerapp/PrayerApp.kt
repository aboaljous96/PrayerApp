...
class AdhanPlayer {
    ...
    fun play(resourcePath: String) {
        stop()
        try {
            val resourceStream: InputStream? = javaClass.classLoader.getResourceAsStream(resourcePath)
            if (resourceStream == null) {
                println("!!! خطأ: لم يتم العثور على ملف الصوت: $resourcePath")
                println("!!! تأكد من وضع ملفات mp3 في مجلد 'src/main/resources'")
                return
            }
...
private fun adhanResForLabel(label: String): String? = when (label) {
    "الظهر"   -> "azan_dhuhr.wav"   // تم التعديل
    "العصر"   -> "azan_asr.wav"     // تم التعديل
    "المغرب"  -> "azan_maghrib.wav" // تم التعديل
    "العشاء"  -> "azan_isha.wav"    // تم التعديل
    "الفجر"   -> "azan_isha.wav"    // تم التعديل (ملف بديل)
    else      -> null
}
...
fun PrayerTimesApp() {
    ...
    LaunchedEffect(nextPrayerInfo, currentDateKey) {
        ...
        if (prev != null && (nextPrayerInfo == null || prev.label != nextPrayerInfo.label || prev.time != nextPrayerInfo.time)) {
            ...
            if (signature != lastPlayedSignature) {
                adhanResForLabel(prev.label)?.let { adhanPlayer.play(it) }
    ...
    val onPrayerCellClick: (String) -> Unit = { label ->
        scope.launch {
            ...
            adhanResForLabel(label)?.let { adhanPlayer.play(it) }
...
fun BigDigitalClockCard(nowMillis: Long, modifier: Modifier = Modifier) {
    ...
    Card(
        ...
        BoxWithConstraints(
            ...
            val brush = remember(pulse, maxWidth, maxHeight) {
                val radiusPx = with(density) { maxWidth.toPx() } * 1.6f
                val center = Offset(
                    x = with(density) { maxWidth.toPx() } / 2f,
                    y = with(density) { maxHeight.toPx() } / 2f
                )
                Brush.radialGradient(
                    colors = listOf(glowColor.copy(alpha = 0.6f * pulse), Color.Transparent),
                    center = center,
                    radius = radiusPx
                )
            }
            ...
            Text(
                ...
                style = TextStyle(
                    ...
                    shadow = Shadow(
                        color = glowColor.copy(alpha = 1f),
                        offset = Offset.Zero,
                        blurRadius = 40f + 12f * pulse
                    )
                )
            )
        }
...
private fun TimeCell(
    ...
    BoxWithConstraints(modifier = modifier) {
        ...
        val heightBasedValueValue = cellHeight.value / 2.8f
        // اختيار القيمة الأصغر (باستخدام kotlin.math.min) ثم تحويلها إلى sp
